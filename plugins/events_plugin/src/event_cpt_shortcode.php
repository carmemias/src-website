 ...
 *
 * @param array $vars Shortcode parameters.
 */
 ...
 * @return string  Returns shortcode html as string.
 */
...
function get_filter_data( $initial_events_cpt ) {
	...
	foreach ( $initial_events_cpt as $single_event ) {
		...
		if ( ! array_key_exists( $event_date_raw, $output_dates ) ) {
			$output_dates[ $event_date_raw ] = $event_date; }

		$event_area     = ucfirst( sanitize_text_field( $single_event->_event_cpt_area ) );
		$event_area_key = str_replace( ' ', '_', strtolower( $event_area ) );
		if ( ( '' !== $event_area ) && ( ! array_key_exists( $event_area_key, $output_areas ) ) ) {
			$output_areas[ $event_area_key ] = $event_area;}
	}
 ...
 *
 * @param array $all_areas All event cities and areas to show on filter dropdown.
 */
 ...
 * Renders the event-area filter dropdown
 *
 * @param array $all_dates All event dates to show on filter dropdown.
 */
 ...
 *
 * @param array $all_types All event types to show on the filter dropdown.
 */
 ...
 *
 * @param array $events_cpt Array of event objects.
 */
function render_programme_page( $events_cpt ) {
	...
	foreach ( $events_cpt as $single_event ) {
		...
		if ( ! $event_date ) {
			$output_view .= '<span style="color: #f00;">No date set yet</span>';
		} else {
			$output_view .= ' <p class="date">' . $event_date . ' ' . $event_start_time . ' - ' . $event_end_time . '</p>';}
...
/**
 * Get all Social Media links for the event.
 *
 * @param  object $event Event Object.
 * @return string        HTML code as a string.
 */
...
/**
 * Render the Event's venue name and city.
 *
 * @param  object $event Event Object.
 * @return string        HTML code for displaying the venue name and city.
 */
